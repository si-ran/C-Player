...
class PacketGrabber {
  ...
  int av_rate;
  TimeoutCount* count = new TimeoutCount;

 public:
     static int interrupt_cb(void* ctx) {
         TimeoutCount* count = (TimeoutCount*)ctx;
         time_t times = time(NULL);
         if (count->count == 0) {
             count->count = times;
         }
         if (count->index == 0 && times - count->count > 5) {
             cout << times - count->count << endl;
             return 1;
         }
         return 0;
     }
  ...
  PacketGrabber(const string& uri) : inputUrl(uri) {
    ...
    formatCtx->interrupt_callback.opaque = count;
    ...
    for (int i = 0; i < formatCtx->nb_streams; i++) {
      ...
      if (formatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO && videoIndex == -1) {
        ...
        ffinfo.width = v_width;
        ffinfo.height = v_height;
        ffinfo.frame_number = video_frame_count;
        ffinfo.frame_rate = frame_rate;
      }
      ...
      if (formatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO && audioIndex == -1) {
        ...
        ffinfo.audio_samplerate = in_stream->codec->sample_rate;
        ffinfo.audio_channels = in_stream->codec->channels;
        cout << "—————————over———————————" << endl;
      }
    }
  }
